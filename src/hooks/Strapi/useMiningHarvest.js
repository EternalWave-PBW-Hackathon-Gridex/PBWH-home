...
export default function useMiningHarvest() {
  /** poolVotes */
  ...
  const fetch = async () => {
    let requestInfo = {
      url: `${URL.BASEURL}/api/${PROJECT_ID}-mining-harvests`,
      params: {
        "pagination[page]": 1,
  ...
  const isValidMiningHarvest = React.useMemo(() => {
    if (!isValidData) return false;
    return attributes.length === 1;
  }, [isValidData, attributes]);

  const miningHarvest = React.useMemo(() => {
    if (!isValidMiningHarvest) return null;
    return attributes[0];
  }, [isValidMiningHarvest, attributes]);

  return {
    data,
    isValidData,
    isLoadingMiningHarvest: isLoading,
    isCallSuccessMiningHarvest: isCallSuccess,
    isValidMiningHarvest,
    miningHarvest,

    fetchMiningHarvest: fetch,
    setLoadingMiningHarvest: setLoading
  };
}
