...
export default function useDailyStakingHarvest() {
  /** poolVotes */
  ...
  const fetch = async () => {
    let requestInfo = {
      url: `${URL.BASEURL}/api/${PROJECT_ID}-daily-staking-harvests`,
      params: {
        "pagination[page]": 1,
  ...
  const isValidDailyStakingHarvest = React.useMemo(() => {
    if (!isValidData) return false;
    return attributes.length === 1;
  }, [isValidData, attributes]);

  const dailyStakingHarvest = React.useMemo(() => {
    if (!isValidDailyStakingHarvest) return null;
    return attributes[0];
  }, [isValidDailyStakingHarvest, attributes]);

  return {
    data,
    isValidData,
    isLoadingDailyStakingHarvest: isLoading,
    isCallSuccessDailyStakingHarvest: isCallSuccess,
    isValidDailyStakingHarvest,
    dailyStakingHarvest,

    fetchDailyStakingHarvest: fetch,
    setLoadingDailyStakingHarvest: setLoading
  };
}
