...
import MeshswapEscrowContract from "../../contracts/MeshswapEscrowContract";
...
export default function useCDepositMESH() {
  const methodName = "depositMESH";
  ...
  const fetchTrxFee = async (...args) => {
    try {
      const gasUnit = MeshswapEscrowContract.estimateGas[methodName](...args, {
        from: address
      });
      const gasPrice = MeshswapEscrowContract.provider
        .getGasPrice()
  ...
  const {
    ...
    fetchTransaction
  } = useTransaction(MeshswapEscrowContract, methodName);
  ...
  return {
    ...
    /** Tx */
    isLoadingDepositMESHTx: isLoadingTx,
    fetchDepositMESHTx: fetchTransaction,

    /** Helpers */
    isValidDepositMESHTx: isValidTx
  };
}
