...
export default function useBIDepositLP() {
  ...
  const fetchTrxFee = async (...args) => {
    try {
      const gasUnit = FundFarmContract.estimateGas[methodName](...args, {
        from: address
      });
      const gasPrice = FundFarmContract.provider
        .getGasPrice()
  ...
  const {
    ...
    fetchTransaction
  } = useTransaction(FundFarmContract, methodName);
  ...
  return {
    /** Tx Fee */
    isCallSuccessDepositLPTxFee: isCallSuccess,
    isLoadingDepositLPTxFee: isLoadingTxFee,
    fetchDepositLPTxFee: fetchTrxFee,
    displayDepositLPTxFee: display,
    setDepositLPTxFeeLoading: setLoading,

    /** Tx */
    isLoadingDepositLPTx: isLoadingTx,
    fetchDepositLPTx: fetchTransaction,

    /** Helpers */
    isValidDepositLPTx: isValidTx
  };
}
