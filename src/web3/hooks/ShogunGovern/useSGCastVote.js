...
export default function useSGCastVote() {
  const methodName = "castVote";
  ...
  const fetchTrxFee = async (...args) => {
    try {
      const gasUnit = ShogunGovernContract.estimateGas[methodName](...args, {
        from: address
      });
      const gasPrice = ShogunGovernContract.provider
        .getGasPrice()
  ...
  const {
    ...
    fetchTransaction: fetchCastVoteTx
  } = useTransaction(ShogunGovernContract, methodName);
  ...
  return {
    /** Tx Fee */
    isCallSuccessCastVoteTxFee: isCallSuccess,
    isLoadingCastVoteTxFee: isLoadingTxFee,
    fetchCastVoteTxFee: fetchTrxFee,
    displayCastVoteTxFee: display,
    setCastVoteTxFeeLoading: setLoading,

    /** Tx */
    isLoadingCastVoteTx: isLoadingTx,
    fetchCastVoteTx,

    /** Helpers */
    isValidCastVoteTx: isValidTx
  };
}
